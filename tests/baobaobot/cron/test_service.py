...
class TestCronServiceExecuteJob:
    ...
    @pytest.fixture
    def service(self, tmp_path: Path, mock_sm: MagicMock) -> CronService:
        ws_dir = tmp_path / "workspace_test"
        ws_dir.mkdir()
        svc = CronService(
    ...
    async def test_execute_job_success(self, service: CronService, mock_sm: MagicMock):
        job = CronJob(
            id="abc",
            name="test",
    ...
    async def test_execute_job_send_failure(
        self, service: CronService, mock_sm: MagicMock
    ):
        job = CronJob(
            id="abc",
            name="test",
    ...
    async def test_execute_job_no_window(
        self, service: CronService, mock_sm: MagicMock
    ):
        job = CronJob(
            id="abc",
        ...
        service._recreate_window = AsyncMock(return_value=None)
    ...
    async def test_execute_at_job_disables_after_run(
        self, service: CronService, mock_sm: MagicMock
    ):
        job = CronJob(
            id="abc",
    ...
    async def test_execute_job_with_cached_window_id(
        self, service: CronService, mock_sm: MagicMock
    ):
        job = CronJob(
            id="abc",
...
class TestCronServiceExecuteDueJobs:
    ...
    async def test_executes_due_jobs(self, service: CronService, mock_sm: MagicMock):
        now = time.time()
        job = CronJob(
            id="abc",
            name="due-job",
    ...
    async def test_skips_disabled_jobs(self, service: CronService, mock_sm: MagicMock):
        now = time.time()
        job = CronJob(
            id="abc",
            name="disabled",
    ...
    async def test_skips_future_jobs(self, service: CronService, mock_sm: MagicMock):
    ...
    async def test_delete_after_run(self, service: CronService, mock_sm: MagicMock):
        now = time.time()
        job = CronJob(
            id="abc",
            name="once",
    ...
    async def test_clears_stuck_job(self, service: CronService, mock_sm: MagicMock):
        now = time.time()
        job = CronJob(
            id="abc",
            name="stuck",
...
class TestCronServiceProperties:
    def test_defaults(self, tmp_path: Path):
        svc = CronService(
            ...
            cron_default_tz="",
            users_dir=tmp_path / "users",
            workspace_dir_for=lambda name: tmp_path,

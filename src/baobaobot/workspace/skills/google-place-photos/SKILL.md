...
curl -s -L "https://places.googleapis.com/v1/$PHOTO_NAME/media?maxHeightPx=600&key=$GOOGLE_MAPS_API_KEY" \
...
for photo in $PHOTOS; do
  curl -s -L "https://places.googleapis.com/v1/$photo/media?maxHeightPx=600&key=$GOOGLE_MAPS_API_KEY" \
...
curl -s -L "https://places.googleapis.com/v1/$PHOTO_NAME/media?maxHeightPx=400&key=$GOOGLE_MAPS_API_KEY" -o tmp/photo.jpg

# By max width (preserves aspect ratio)
curl -s -L "https://places.googleapis.com/v1/$PHOTO_NAME/media?maxWidthPx=800&key=$GOOGLE_MAPS_API_KEY" -o tmp/photo.jpg

# Both (fits within bounds)
curl -s -L "https://places.googleapis.com/v1/$PHOTO_NAME/media?maxHeightPx=600&maxWidthPx=800&key=$GOOGLE_MAPS_API_KEY" -o tmp/photo.jpg
```
...
| `key` | API key | `$GOOGLE_MAPS_API_KEY` |
